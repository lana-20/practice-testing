...
| Coffee Cart | https://coffee-cart.app/ | Clean Vue SPA; 9 drinks listed (Espresso $10–Cappuccino $19); product cards are `div[data-test=ProductName]` — not in `vibium map`; click via `eval getBoundingClientRect()` + `vibium mouse click x y`; cart nav at `/cart` shows items with `+`/`-`/`×` buttons — all accessible via `vibium map` and `vibium click`; checkout modal: name + email required (HTML5 validation), promo checkbox optional; successful submit → "Thanks for your purchase. Please check your email for payment." and cart resets to 0; full flow works end-to-end |
