...
| QA Practice | https://qa-practice.razvanvancea.ro/ | Login: `admin@admin.com` / `admin123`; ADD TO CART uppercase — use map refs; pre-stub alert/confirm before clicking alert buttons (B3 still open); checkout: Shipping Details form (phone, street, city, country) | MB3 deadlock on direct Alert button click — use `browser_evaluate {setTimeout(...,300)}` + `browser_sleep {ms:350}` + `browser_dialog_accept {}`; full e-commerce flow works; country select: label = display text exactly |
