...
| Commit Quality | https://commitquality.com/ | React app; `input[type=date]` not fillable via `vibium fill` — use `vibium type` on fresh field; date filter rejects today's date ("must not be in the future") — use past dates; Login credentials not public; Add Product form validates required fields |
| Contact List App | https://thinking-tester-contact-list.herokuapp.com/ | Heroku; full CRUD works; edit form has no input placeholders — fill by index via eval; delete triggers `window.confirm` — pre-stub before clicking Delete; registration creates persistent account |
| Demo SaaS | https://demo-saas.bugbug.io/ | Email verification required after signup — dashboard not accessible without real email; landing page and validation flows testable; all forms found by `vibium map` |
| GreenKart | https://rahulshettyacademy.com/seleniumPractise/#/ | Scrolling ticker banner intercepts nav clicks — use `vibium eval 'location.href="..."'` for nav; search: `vibium press Enter "input[type=search]"` (not button click); ADD TO CART: requires `vibium scroll "into-view"` first; PROCEED: use eval querySelectorAll button find; drag: use `mouse move/down/up` coords (not `vibium drag @ref`); Bugs: "Quantiry" typo in cart header, silent promo code failure, Rice discount price > regular price on Top Deals |
| Global SQA Demo | http://www.globalsqa.com/demo-site/ | HTTP auto-redirects to HTTPS (no block); all component links on main page redirect to ad (`#google_vignette`) — navigate sub-pages directly; all demo widgets in iframes — get iframe URL via `vibium frames`, navigate there directly (`vibium frame` doesn't switch CLI context); `vibium drag @ref` fails on non-interactive targets — use `mouse move/down/up` coords; E-CommerceSite link is broken (points to same page); Confirmation Box `myFunctionTab2()` throws in eval even after `window.confirm` override |
| Hands-On Selenium WebDriver | https://bonigarcia.dev/selenium-webdriver-java/ | 30 practice sub-pages at clean URLs; `vibium fill` fails on `<textarea>` — use `vibium type` instead; calculator buttons are `span.btn` not `button` — use eval find; login page slow readyState — add `sleep 3` after submit before `vibium url`; no login required, no ads |
| Lambdatest Playground | https://ecommerce-playground.lambdatest.io/ | OpenCart store; all nav links obscured by sticky header — use direct URLs; Add to Cart silently fails for guests — login required; products with "Size" select have no actual options (data issue, "Size required!" blocks add to cart); out-of-stock items can be added to cart (appear with `***` warning); `vibium mouse click x y` at button coords works; `window.cart.add(productId, qty)` is the AJAX function |
| Let Code | https://letcode.in/test | Angular app; all hub links redirect to `#google_vignette` — navigate sub-pages via direct URL (e.g. `letcode.in/edit`); `/alert` page deadlocks daemon on load (native confirm fires immediately) — avoid or use MCP dialog_accept; `vibium fill` works on text inputs; `vibium select` works on dropdowns; `vibium type` needed for any textarea fields |
| Locator Game | https://testsmith-io.github.io/locator-game/ | Static GitHub Pages; 13 levels (0–12) of CSS/XPath locator challenges; dismiss tutorial with "End tour" button; `vibium fill @e3 "h3"` + `vibium click @e5` (Submit) to answer; correct answer advances level immediately; CSS and XPath modes via `vibium select @e2 "XPath"` |
| Practice Test Automation | https://practicetestautomation.com/practice/ | 3 practice pages: Login (`/practice-test-login/`), Exceptions (`/practice-test-exceptions/`), Table (`/practice-test-table/`); login: `student`/`Password123`; add `sleep 5` before `vibium url` after successful login submit (BiDi timing issue); table has language radio + level checkbox filters; exceptions page adds Row 2 dynamically after Add click (use `sleep 2`); no ads, no login required for browsing |
| QA Playground | https://qaplayground.dev/ | 24 mini-apps at `/apps/<slug>/`; OTP (Verify Account) requires `vibium type` not `fill`; shadow DOM: `document.querySelector("progress-bar")?.shadowRoot?.querySelector("button")?.click()`; sortable list — rearrange `.draggable` children via eval; range slider — `eval .value=N + dispatchEvent(input/change)`; onboarding modal — CSS checkbox toggle (`#active`); no `vibium switch` for new tabs — navigate directly to new page URL; `vibium find "text"` returns nil on most mini-apps — use `vibium map` |
| React Shopping Cart | https://react-shopping-cart-67954.firebaseapp.com/ | Firebase React SPA; 16 products; size filter checkboxes obscured — use `eval .click()` not `vibium check`; cart drawer opened by clicking count button (last elem in `vibium map`); no real checkout (frontend demo only) |
| Selectors Hub | https://selectorshub.com/xpath-practice-page/ | XPath/CSS practice page; email input has `readonly` by default — `eval .removeAttribute("readonly")` before filling; open shadow DOM on `#userName`, closed on `#userPass`; nested shadow DOM inside shadow DOM; iframe-in-shadow and shadow-in-iframe scenarios |
| Selenium Playground | https://www.lambdatest.com/selenium-playground/ | Index page loads at lambdatest.com but all 40+ demo links redirect to testmuai.com; testmuai.com blocked by Cloudflare bot protection — not accessible to automation (as of 2026-05-18) |
| Swag Labs | https://www.saucedemo.com/ | credentials: standard_user/secret_sauce; 6 user types; full checkout flow works; cart icon not in `vibium map` — use `eval document.querySelector(".shopping_cart_container a")?.click()`; `vibium go cart.html` fails with BiDi error after logout — click cart from inventory; sort values: "az", "za", "lohi", "hilo"; problem_user: all images broken (sl-404), sort non-functional (intentional bugs); `vibium find "Finish"` returns nil — use `vibium map` |
| Sweet Shop | https://sweetshop.netlify.app/ | Static Netlify site; "Add to Basket" links have no `href` and are not in `vibium map` — use `eval document.querySelector(".addItem").click()` or `document.querySelector(".addItem[data-id=\"N\"]").click()`; checkout form has two inputs both with `id="name"` (First + Last name) — `vibium fill` only reaches first; fill second via `eval document.querySelectorAll("input#name")[1].value="User"`; delivery radio obscured — use `eval .click()`; form submits as GET (no backend, page reloads at `/basket?`); login has no credentials and no feedback |
| Tricentis Obstacle Course | https://obstaclecourse.tricentis.com/Obstacles | Obstacles randomized per load; text-answer: `vibium fill @eN + vibium press "Enter"`; date obstacle: click #generate, compute first of 2nd following month in ISO, fill #dateSolution + click #done; table: find row by text via eval + click querySelectorAll("button")[1]; success modal is obscured — use `eval document.querySelector("button.btn-success")?.click()`; "next one" link: `eval document.querySelector("a[href*=next]").click()`; drag-and-drop obstacles unresponsive to mouse events + HTML5 DragEvent API |
| UI Test Automation Playground | http://uitestingplayground.com/ | HTTP-only site — Chrome BiDi blocks navigation completely; `eval location.href` → `chrome-error://chromewebdata/`; accessible via curl only; cannot be tested with vibium (same as Testing Challenges) |
| Weather Shopper | https://weathershopper.pythonanywhere.com/ | Temperature-based product selection; Add buttons update cart count button; cart at `/cart`; Stripe TEST MODE modal opens on "Pay with Card" click (cross-origin iframe — not fillable via eval); Stripe fields in stripe_checkout_app iframe; `vibium frames` lists iframe; test card: 4242 4242 4242 4242 / 12/26 / 123 |
| XYZ Bank | https://www.globalsqa.com/angularJs-protractor/BankingProject/ | AngularJS SPA; `vibium select` doesn't trigger ng-model binding — use `eval .value="N" + dispatchEvent(new Event("change",{bubbles:true}))`; Add Customer submit fires `window.alert()` — pre-stub before click; deposit/withdrawal/transactions work; manager panel: Add Customer, Open Account, Customers list with search and Delete |
