...
| QA Training Simulator | https://bugeater.web.app/ | 25 challenges (Learn Mode, Scripted, Gherkin, Functional, API, Exploratory); use `vibium type` not `vibium fill` for React controlled inputs; challenge URLs require navigation from `/app/list` not direct URL (some routes crash React with TypeError on direct load); dismiss tutorial overlay before clicking form buttons |
| Random User Generator | https://randomuser.me/ | API testing site; nav links broken when clicked from homepage — use `vibium go <url>` directly; API at `/api/` works; `?results=0` and `?results=abc` silently return 1 result; `?format=xml` works |
| Real World Example Apps | https://codebase.show/projects/realworld | Directory of RealWorld Conduit implementations; Sign in + Submit nav links reload page instead of redirecting — use `vibium go https://codebase.show/sign-in` directly; requires GitHub OAuth |
| The Boozang Test Lab | https://thelab.boozang.com/ | React SPA with 16 challenges; many buttons require `eval.click()` not `vibium click`; Form Fill has no required field validation (empty saves to shared DB); `vibium select` correct option values via inspect (`#engines` select) |
| The iframe Search Engine | http://compendiumdev.co.uk/apps/iframe-search/iframe-search.html | Vanilla JS; Search button onclick doesn't fire from `vibium click` — use `vibium eval 'searchTool.changeUrlFromForm()'`; use `vibium fill` (not type) for search input; B5 confirmed: `vibium select` silently ignores non-existent option values; Bing embeds in iframe, Google/DuckDuckGo block it |
